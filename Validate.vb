...
Module Validate
                ...
                For inner = outer + 1 To theWorld.Count - 1
                    ...
                    If Not Acceptable Then
                        If GeometryEngine.Intersects(theWorld.Item(outer).geom, theWorld.Item(inner).geom) Then
                            AppendText(Form1.TextBox1, $"The entities {theWorld.Item(outer).Entity} and {theWorld.Item(inner).Entity} overlap{vbCrLf}")
                            overlaps += 1
                        End If
                    End If
                Next
    ...
    Sub IOTACheck()
        ...
        Using connect As New SqliteConnection(DXCC_DATA)
            ...
            While SQLdr.Read
                ...
                SQLdr1.Read()
                AppendText(Form1.TextBox1, $"Checking IOTA {SQLdr("refno")} and {SQLdr1("Entity")}{vbCrLf}")
                Dim geometry As Polygon = GeoJsonToGeometry(SQLdr1("geometry"))      ' retrieve the DXCC geometry
                ' Now check for intersection of IOTA and DXCC
                If Not geometry.Intersects(iota) Then
                    AppendText(Form1.TextBox1, $"************** IOTA ref {SQLdr("refno")} - {SQLdr("name")} does not intersect with DXCC {SQLdr1("Entity")} **************{vbCrLf}")
                End If
                SQLdr1.Close()
            End While
        End Using
    ...
    Public Sub AdjacentColourCheck()
        ...
        Using connect As New SqliteConnection(DXCC_DATA)
            ...
            sqldr = sql.ExecuteReader
            While sqldr.Read
                countries.Add((prefix:=sqldr("prefix"), geom:=GeoJsonToGeometry(sqldr("geometry")), colour:=sqldr("colour")))
            End While
        End Using
        ...
        For outer = 0 To countries.Count - 2
            ...
            For Each item In adjacent
                If item.colour = countries(outer).colour Then AppendText(Form1.TextBox1, $"Adjacent countries {countries(outer).prefix} and {item.prefix} share a colour{vbCrLf}")
            Next
        Next
        AppendText(Form1.TextBox1, $"Done{vbCrLf}")
    End Sub
